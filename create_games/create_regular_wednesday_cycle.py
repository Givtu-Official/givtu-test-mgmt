...
fortune_draws = [
    {"draw_amount": 200000, "draw_date": "Sunday", "draw_amount_reset": 100000, "fortune_group": 0,
     "draw_time_hour": 18}
]
...
def generate_draw_schedule_entries(count=8):
    entries = []
    ...
    for i in range(count):
        if current_day_name == "Wednesday":
            wednesday_count %= 4
            wednesday_count += 1
            name = f"Wednesday {wednesday_count}"
            end_date = current_start + timedelta(days=7)  # Friday
        ...
        days_to_add = 7 if current_day_name == "Wednesday" else 5
...
def main():
    ...
    draw_schedule_entries = generate_draw_schedule_entries(8)
    ...
    for i in range(len(fortune_draws), fortune_draws_total):
        print('-----------fortune_draws------------')
        print(fortune_draws)
        last_draw_date = datetime.strptime(fortune_draws[i - 1]["draw_date"], '%Y-%m-%d %H:%M:%S') + timedelta(days=7)
